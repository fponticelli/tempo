...
import { moduleFromSourceFile, Module, empty } from './parse/module'
...
const processProject = (destPath: string, basePath: string, renderModule: (module: Module) => string) => (name: string) => {
...
const makeView = () => {
  ...
  const view = module.render(ctx, empty)
  const getHtml = () => {
