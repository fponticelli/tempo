...
import { Project, Layer, Shape } from 'paper'
...
export const project = <State, Action>(
  ...
  return el<State, Action, HTMLCanvasElement, PaperScope<State, Action>>(
    'canvas',
    {
      ...
      afterrender: (state: State, el: HTMLCanvasElement, ctx) => {
        const project = new Project(el)
        const rootLayer = new Layer()
        project.addLayer(rootLayer)
        const context = new PaperContext(project, (shape: Shape) => rootLayer.addChild(shape))
