...
export interface InputAttributes<State, Action> {
    ...
    type?: DOMAttribute<State, 'button' | 'checkbox' | 'color' | 'date' | 'datetime-local' | 'email' | 'file' | 'hidden' | 'image' | 'month' | 'number' | 'password' | 'radio' | 'range' | 'reset' | 'search' | 'submit' | 'tel' | 'text' | 'time' | 'url' | 'week'>;
...
export declare function input<State, Action>(attributes: InputAttributes<State, Action> & CSSAttributes<State> & MoodAttributes<State, HTMLInputElement>, ...children: DOMChild<State, Action>[]): DOMElement<State, Action>;
